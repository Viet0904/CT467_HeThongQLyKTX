...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$oldExists){
            // Thêm mới
            $stmt = $dbh->prepare("CALL InsertPhong(:MaPhong, :MaDay, :TenPhong, :LoaiPhong, :DienTich, :SucChua, :SoChoThucTe, :DaO, :GiaThue, :TrangThaiSuDung)");
            $stmt->execute([
                ':MaPhong' => $maPhong,
                ':MaDay' => $maDay,
                ':TenPhong' => $tenPhong,
            ...
            if ($oldMaPhong !== $maPhong) {
                if ($newExists) {
                    ...
                    $stmt = $dbh->prepare("CALL UpdatePhong(:MaPhong, :OldMaPhong, :MaDay, :TenPhong, :LoaiPhong, :DienTich, :SucChua, :SoChoThucTe, :DaO, :GiaThue, :TrangThaiSuDung)");
                    $stmt->execute([
                        ':MaPhong' => $maPhong,
